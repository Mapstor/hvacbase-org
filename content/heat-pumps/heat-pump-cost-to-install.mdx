---
slug: "heat-pump-cost-to-install"
title: "Heat Pump Installation Cost: Complete Pricing Guide (2026)"
description: "Heat pump installation costs $4,500–$12,500 in 2026 for a ducted system, or $2,700–$19,000 for mini splits. See full cost breakdowns by type, brand, region, and system size."
cluster: "heat-pumps"
role: "spoke"
priority: "P1"
contentType: "cost-guide"
datePublished: "2026-02-05"
dateModified: "2026-07-18"
relatedArticles:
  - "heat-pump-guide"
  - "heat-pump-tax-credits-2026"
  - "best-mini-split-heat-pumps"
  - "heat-pump-electricity-usage"
  - "heat-pump-running-cost-calculator"
  - "disadvantages-of-heat-pumps"
  - "heat-pump-vs-ac"
  - "air-source-vs-ground-source-heat-pump"
externalLinks:
  - label: "ACCA Contractor Locator"
    url: "https://www.acca.org/contractors"
  - label: "IRS 25C Tax Credit"
    url: "https://www.irs.gov/credits-deductions/energy-efficient-home-improvement-credit"
---

import { TableOfContents, FAQ, ComparisonTable, Callout, SourcesBox, RelatedArticles } from '@/components'

# Heat Pump Installation Cost: Complete Pricing Guide (2026)

The average heat pump installation costs $5,800–$10,000 in 2026 for a 3-ton ducted air-source system, including equipment and professional labor. (Note: the federal Section 25C credit — which covered up to $2,000 for qualifying heat pumps — expired for property placed in service after Dec 31, 2025 under the OBBBA; state and utility rebates plus IRA HEAR / HOMES remain the active 2026 federal pathways.) A single-zone mini split runs $2,700–$5,800 installed, while a ground-source (geothermal) system costs $18,000–$35,000 before incentives. Geothermal's 30% federal credit (Section 25D) also expired for installs after Dec 31, 2025.

These numbers represent national averages from contractor pricing surveys in January 2026. Your actual cost depends on system type, capacity, brand, installation complexity, your region, and whether you need electrical upgrades or new ductwork.

<TableOfContents>
- [Quick Cost Summary](#quick-cost-summary)
- [Cost by System Type](#cost-by-system-type)
- [Cost by Brand](#cost-by-brand)
- [Cost by System Size](#cost-by-system-size)
- [Labor Cost Breakdown](#labor-cost-breakdown)
- [What Increases Installation Cost](#what-increases-cost)
- [Cost by Region](#cost-by-region)
- [Tax Credits and Rebates That Lower Your Cost](#tax-credits-rebates)
- [How to Get the Best Price](#how-to-get-best-price)
- [Key Takeaways](#key-takeaways)
- [FAQ](#faq)
</TableOfContents>

## Quick Cost Summary 

<ComparisonTable
  title="2026 Heat Pump Installation Cost Overview"
  headers={["System Type", "Total Installed Cost", "After State/Utility Rebates*", "Typical Payback Period"]}
  rows={[
    ["Ducted Air-Source (standard)", "$4,500–$8,500", "$3,500–$8,000", "5–8 years"],
    ["Ducted Air-Source (premium)", "$8,000–$12,500", "$7,000–$12,000", "6–10 years"],
    ["Mini Split (single zone)", "$2,700–$5,800", "$1,700–$5,300", "3–6 years"],
    ["Mini Split (multi-zone, 3–4 heads)", "$9,500–$19,000", "$8,500–$18,500", "6–9 years"],
    ["Mini Split DIY (single zone)", "$1,400–$2,200", "$1,400–$2,200*", "1–4 years"],
    ["Ground-Source / Geothermal", "$18,000–$35,000", "$17,000–$32,000**", "12–20 years"],
  ]}
/>

*State/utility rebate column shows typical post-rebate cost for 2026 installs using state/utility programs only. Income-qualified households can also stack IRA HEAR (up to $8,000 for heat pumps) for substantially lower net cost. The federal Section 25C credit ($2,000) is no longer in this stack — it expired Dec 31, 2025 under the OBBBA (PL 119-21, signed July 4, 2025). DIY installations historically didn't qualify for 25C either.*

**Geothermal previously qualified for the Section 25D 30% (no-cap) credit; that credit also expired for installs after Dec 31, 2025 under the OBBBA. 25D allows carryforward of an unused pre-2026 credit (25C does not). Geothermal-specific state rebates and IRA HOMES remain the active 2026 pathways.**

## Cost by System Type 

### Ducted Air-Source Heat Pump

This is the most common type, replacing your existing furnace and AC while using your home's ductwork. Equipment cost ranges from $2,200 to $7,000 depending on brand and efficiency tier. Installation labor runs $2,500–$5,000.

**Standard efficiency (SEER2 15–17, HSPF2 8–9):** These meet federal minimums and ENERGY STAR thresholds. (Pre-expiration, this tier fell below the SEER2 ≥ 18 / HSPF2 ≥ 9.5 threshold for the now-expired federal 25C credit; the 25C credit expired Dec 31, 2025 under the OBBBA for all efficiency tiers regardless. For 2026 installs, IRA HEAR (income-qualified, up to $8,000 for heat pumps) and HOMES (performance-based) plus state/utility rebates often still require the SEER2 18+ / HSPF2 9.5+ tier for full benefit.) Total installed cost: $4,500–$8,000.

**High efficiency (SEER2 18–22, HSPF2 9.5–11+):** Through Dec 31, 2025, these qualified for the full $2,000 federal 25C tax credit; for 2026 installs the 25C credit has expired (OBBBA), but most state and utility rebate programs (and IRA HEAR / HOMES) still use SEER2 18+ / HSPF2 9.5+ as eligibility thresholds. Variable-speed compressors deliver better comfort and efficiency. Total installed cost: $7,500–$12,500.

**Cold-climate rated (SEER2 18+, HSPF2 10+, rated to −13 °F or below):** Premium units from Mitsubishi, Fujitsu, Daikin, and Bosch designed for zones 5–7. Total installed cost: $8,000–$13,000.

### Ductless Mini Split

Mini splits skip ductwork entirely, connecting an outdoor compressor to one or more wall-mounted indoor heads via refrigerant lines.

<ComparisonTable
  title="Mini Split Installation Costs by Configuration"
  headers={["Configuration", "Equipment", "Labor", "Total Installed"]}
  rows={[
    ["Single zone (9K–12K BTU)", "$1,200–$3,500", "$1,500–$2,500", "$2,700–$6,000"],
    ["Single zone (18K–24K BTU)", "$1,800–$4,500", "$1,800–$3,000", "$3,600–$7,500"],
    ["2-zone multi-split", "$3,000–$6,000", "$3,500–$5,000", "$6,500–$11,000"],
    ["3-zone multi-split", "$4,500–$8,000", "$5,000–$7,000", "$9,500–$15,000"],
    ["4-zone multi-split", "$6,000–$10,000", "$6,000–$9,000", "$12,000–$19,000"],
    ["5-zone multi-split", "$7,500–$12,000", "$7,000–$10,000", "$14,500–$22,000"],
  ]}
/>

### Ground-Source (Geothermal)

Geothermal systems exchange heat with the earth through buried pipes. The equipment itself costs $5,000–$8,000, but the ground loop installation (drilling or trenching) makes up the bulk of the cost.

**Horizontal loop (trenched):** Requires 400–600 feet of trench per ton. Works in large, flat yards. Total: $18,000–$28,000 for a 3-ton system.

**Vertical loop (bored wells):** Requires 150–200 feet of depth per ton. Works in smaller yards. Total: $22,000–$35,000 for a 3-ton system.

**Open loop (well water):** Uses groundwater directly. Cheapest to install but requires adequate water supply and a discharge strategy. Total: $15,000–$22,000 for a 3-ton system.

For a full geothermal-vs-air-source ROI analysis (efficiency, lifespan, and payback by climate zone), see our [air-source vs ground-source heat pump comparison](/air-source-vs-ground-source-heat-pump/).

## Cost by Brand 

Brand premiums vary significantly. Here's what you'll typically pay for a 3-ton ducted system, installed.

<ComparisonTable
  title="2026 Heat Pump Cost by Brand (3-ton Ducted, Installed)"
  headers={["Brand", "Tier", "Total Installed Cost", "SEER2 Range", "Warranty (Compressor)"]}
  rows={[
    ["Carrier / Bryant", "Premium", "$8,000–$12,500", "16–22", "10 years"],
    ["Trane / American Standard", "Premium", "$7,500–$12,000", "15–22", "10 years (12 for XL)"],
    ["Lennox", "Premium", "$8,500–$13,000", "16–24", "10 years"],
    ["Mitsubishi (ducted)", "Premium", "$9,000–$13,500", "18–20", "12 years"],
    ["Daikin / Amana / Goodman", "Mid-Range", "$5,500–$9,500", "15–20", "10–12 years"],
    ["Bosch", "Mid-Range", "$6,500–$10,000", "18–20", "10 years"],
    ["Rheem / Ruud", "Value", "$5,000–$8,500", "15–18", "10 years"],
    ["York / Johnson Controls", "Value", "$4,500–$8,000", "14–18", "10 years"],
  ]}
/>

<Callout type="tip">
**Brand vs. Installer:** In HVAC, the quality of the installation matters more than the brand of the equipment. A mid-range Daikin installed by a skilled, detail-oriented contractor will outperform a premium Carrier installed by a rushed crew. Prioritize contractor reputation over brand name.
</Callout>

## Cost by System Size 

Heat pump cost scales with capacity. Larger homes need larger systems (more tons = more BTUs = higher cost).

<ComparisonTable
  title="Ducted Heat Pump Cost by Size (Mid-Range Brand, 2026)"
  headers={["Size", "BTU Capacity", "Typical Home", "Equipment", "Installation", "Total"]}
  rows={[
    ["1.5 ton", "18,000", "600–1,000 sq ft", "$1,800–$3,500", "$2,200–$3,500", "$4,000–$7,000"],
    ["2 ton", "24,000", "1,000–1,400 sq ft", "$2,200–$4,200", "$2,500–$4,000", "$4,700–$8,200"],
    ["2.5 ton", "30,000", "1,400–1,800 sq ft", "$2,500–$4,800", "$2,800–$4,200", "$5,300–$9,000"],
    ["3 ton", "36,000", "1,800–2,200 sq ft", "$2,800–$5,500", "$3,000–$4,500", "$5,800–$10,000"],
    ["3.5 ton", "42,000", "2,200–2,700 sq ft", "$3,200–$6,000", "$3,200–$4,800", "$6,400–$10,800"],
    ["4 ton", "48,000", "2,700–3,200 sq ft", "$3,500–$6,500", "$3,500–$5,000", "$7,000–$11,500"],
    ["5 ton", "60,000", "3,200–4,000 sq ft", "$4,500–$7,500", "$3,800–$5,500", "$8,300–$13,000"],
  ]}
/>

## Labor Cost Breakdown 

Installation labor typically represents 40–55% of the total project cost. Here's what you're paying for.

<ComparisonTable
  title="Installation Labor Components"
  headers={["Task", "Cost Range", "Time Required", "Notes"]}
  rows={[
    ["Remove old equipment", "$200–$500", "1–3 hours", "Includes refrigerant recovery"],
    ["Set outdoor unit", "$300–$600", "1–2 hours", "Pad, mounting, vibration isolators"],
    ["Install air handler / coil", "$500–$1,200", "2–4 hours", "Includes refrigerant piping"],
    ["Refrigerant line set", "$300–$800", "1–3 hours", "Length-dependent pricing"],
    ["Electrical connection", "$300–$800", "1–3 hours", "Disconnect, whip, wiring"],
    ["Thermostat install + wiring", "$100–$300", "30 min–1 hour", "Smart thermostat may cost extra"],
    ["System commissioning", "$200–$400", "1–2 hours", "Charge, test, verify performance"],
    ["Permits + inspection", "$100–$500", "Varies", "Required by most municipalities"],
    ["Ductwork modification", "$0–$2,000", "0–4 hours", "If existing ducts need adjustment"],
  ]}
/>

## What Increases Installation Cost 

Several factors can push your total well above the base price.

**Electrical panel upgrade:** Homes with 100A panels often need a 200A upgrade to accommodate the heat pump's 30–50A circuit. Cost: $1,500–$4,000.

**New ductwork:** If your home lacks ducts or the existing ductwork is severely deteriorated, new ducts cost $2,000–$5,000 for a single-story and $4,000–$8,000 for a multi-story home.

**Difficult access:** Second-story installations, tight crawl spaces, or attic air handler placements increase labor time and cost by $500–$1,500.

**Long refrigerant line runs:** Standard installations assume 15–25 feet of line set. Each additional 10 feet adds $150–$300.

**Code-required upgrades:** Some jurisdictions require a condensate pump, overflow switch, secondary drain pan, or seismic bracing that adds $100–$500.

**Crane or rigging:** Rooftop installations or tight-access properties may require a crane ($500–$1,500) to place the outdoor unit.

## Cost by Region 

Labor rates and market conditions vary significantly across the U.S. The same system and installation can differ by 30–40% depending on where you live.

<ComparisonTable
  title="Regional Cost Multipliers (vs National Average)"
  headers={["Region", "Cost Multiplier", "3-ton Ducted (Installed)", "Key Factors"]}
  rows={[
    ["Northeast (NY, NJ, CT, MA)", "1.15–1.30×", "$7,500–$13,000", "High labor rates, strong demand"],
    ["Southeast (GA, FL, NC, SC)", "0.85–1.00×", "$5,000–$9,000", "Competitive market, lower labor"],
    ["Midwest (IL, OH, MI, MN)", "0.95–1.10×", "$5,500–$10,500", "Moderate labor, cold-climate needs"],
    ["Southwest (TX, AZ, NM)", "0.90–1.05×", "$5,200–$9,500", "High cooling demand, moderate labor"],
    ["Mountain West (CO, UT, MT)", "1.00–1.15×", "$6,000–$11,000", "Moderate labor, altitude adjustments"],
    ["Pacific Northwest (WA, OR)", "1.05–1.20×", "$6,500–$11,500", "Strong demand, incentive-driven market"],
    ["California", "1.20–1.40×", "$8,000–$14,000", "Highest labor rates, strict codes"],
  ]}
/>

## Tax Credits and Rebates That Lower Your Cost 

The effective cost of a heat pump can be significantly lower than the sticker price once you apply available 2026 incentives.

**Federal Section 25C Tax Credit — EXPIRED for installs after Dec 31, 2025.** The credit (30% of total project cost up to $2,000 per year for qualifying heat pumps: SEER2 ≥ 16, HSPF2 ≥ 9.0 for ducted; SEER2 ≥ 18, HSPF2 ≥ 9.5 for ductless) expired under the OBBBA (PL 119-21, signed July 4, 2025). 25C does NOT allow carryforward of an unused credit.

**Federal Section 25D Tax Credit (Geothermal) — also EXPIRED Dec 31, 2025.** The 30% (no-cap) credit for geothermal heat pumps expired under the same OBBBA repeal. Carryforward of an unused pre-2026 25D credit IS allowed (unlike 25C).

**IRA HEAR (formerly HEEHRA) — ACTIVE in 2026:** Up to $8,000 point-of-sale rebate, income-dependent. State-administered; available in 40+ states as of 2026.

**IRA HOMES — ACTIVE in 2026:** Open to all incomes, performance-based rebates tied to modeled or measured energy savings. State-administered.

**State + Utility Rebates:** Range from $500 to $16,000 depending on your state and utility. See our complete [tax credits and rebates guide](/heat-pump-tax-credits-2026/). (Sources: IRS OBBB FAQ; Congress.gov CRS IN12611.)

<ComparisonTable
  title="Example 2026 After-Incentive Costs (3-ton Ducted, $9,000 Base — Federal 25C EXPIRED Dec 31, 2025)"
  headers={["Scenario", "Base Cost", "Federal 25C Credit", "State/Utility Rebate", "IRA HEAR (income-qualified)", "Net Cost"]}
  rows={[
    ["No incentives", "$9,000", "$0 (EXPIRED)", "$0", "$0", "$9,000"],
    ["State/utility only (moderate)", "$9,000", "$0 (EXPIRED)", "−$2,500", "$0", "$6,500"],
    ["State/utility only (strong state — MA)", "$9,000", "$0 (EXPIRED)", "−$10,000", "$0", "−$1,000*"],
    ["IRA HEAR only (income-qualified)", "$9,000", "$0 (EXPIRED)", "$0", "−$8,000", "$1,000"],
    ["State/utility + IRA HEAR stacked (income-qualified, moderate state)", "$9,000", "$0 (EXPIRED)", "−$2,500", "−$8,000", "−$1,500*"],
  ]}
/>

*Negative net cost means the incentives exceed the project cost. Some programs cap the total rebate at the project cost; check your specific program rules. The federal Section 25C credit (which previously covered up to $2,000 of qualifying heat pump installs) expired for property placed in service after Dec 31, 2025 under the OBBBA (PL 119-21, signed July 4, 2025). For 2024/2025 installs that didn't claim 25C at the time, an amended return (IRS Form 1040-X) may still recapture the credit — 25C does NOT allow carryforward. (Sources: IRS OBBB FAQ; Congress.gov CRS IN12611.)*

## How to Get the Best Price 

**Get at least 3 quotes.** Pricing varies widely between contractors — quotes for identical systems commonly range from $6,500 to $12,000 in the same market. Three quotes give you a clear picture of the fair market rate.

**Ask what's included.** A low quote that excludes the thermostat, disconnect, line set cover, and permits isn't actually cheap. Request itemized quotes that break out equipment, labor, materials, permits, and warranty terms.

**Time your installation wisely.** Spring (March–May) and fall (September–November) are the sweet spots. Contractors are less busy than during peak summer/winter demand, and many offer shoulder-season discounts of 5–15%.

**Consider mid-range brands.** Daikin, Bosch, and Rheem offer excellent performance at 20–30% lower cost than Carrier, Trane, and Lennox. The efficiency gap is often marginal (1–2 SEER2 points).

**Stack every 2026 incentive.** State rebate + utility rebate + IRA HEAR (income-qualified, up to $8,000 for heat pumps) or HOMES (performance-based) + manufacturer rebate can typically be combined. The federal Section 25C credit that previously added another $2,000 expired Dec 31, 2025 under the OBBBA. Your HVAC contractor should know which 2026 programs you qualify for.

**Finance strategically.** Many utilities and state programs offer 0% or low-interest financing for heat pump installations. Some contractor financing programs charge 12–18% interest — compare all options before signing.

<Callout type="takeaway" id="key-takeaways">
## Key Takeaways

A ducted air-source heat pump costs **$5,800–$10,000 installed** in 2026 for a 3-ton system. The federal Section 25C credit (which covered up to $2,000) **expired for installs after Dec 31, 2025** under the OBBBA, so it no longer offsets 2026 cost. State and utility rebates ($500–$16,000) plus IRA HEAR (income-qualified, up to $8,000) and HOMES (open to all incomes, performance-based) are the active 2026 federal/state pathways. **Electrical panel upgrades** ($1,500–$4,000) and **new ductwork** ($2,000–$8,000) are the most common cost-increasing factors. Regional pricing varies by 30–40%, with California and the Northeast at the high end. Get at least 3 itemized quotes, time installation for shoulder season, and stack every available incentive. (Sources: IRS OBBB FAQ; Congress.gov CRS IN12611.)
</Callout>

<FAQ id="faq">
  <FAQ.Item question="Is it cheaper to install a heat pump or a furnace and AC?">
    A gas furnace + AC combo costs $4,500–$6,500 installed, versus $5,800–$10,000 for a heat pump. The federal 25C credit (which covered up to $2,000 for qualifying heat pumps) expired for installs after Dec 31, 2025, so it no longer helps close that gap. State and utility rebates plus IRA HEAR / HOMES often still narrow the difference. The heat pump also saves $500–$1,200/year on operating costs, making it the cheaper long-term choice in most cases.
  </FAQ.Item>

  <FAQ.Item question="How long does heat pump installation take?">
    A standard ducted heat pump replacement takes 1–2 days. A single-zone mini split takes 4–8 hours. A multi-zone mini split system takes 2–3 days. A geothermal system takes 1–2 weeks (mostly for the ground loop). If an electrical panel upgrade is needed, add 1 day. Permit acquisition may add 1–4 weeks before work begins.
  </FAQ.Item>

  <FAQ.Item question="Can I finance a heat pump installation?">
    Yes. Common financing options include utility-sponsored 0% financing (available from many electric utilities), PACE (Property Assessed Clean Energy) loans repaid through property taxes, manufacturer financing through dealers (often 0% for 12–24 months), home equity loans or lines of credit, and personal loans. Compare interest rates carefully — some dealer financing programs charge 12–18% after the promotional period.
  </FAQ.Item>

  <FAQ.Item question="How much does it cost to add a heat pump to an existing furnace?">
    Adding a heat pump to work alongside your existing gas furnace (a "dual-fuel" or hybrid system) costs $3,500–$7,000 for the heat pump equipment and installation. You keep your furnace as backup for the coldest days, and the heat pump handles heating in moderate weather plus all cooling. This is a popular option in cold climates since it avoids the cost of replacing a working furnace.
  </FAQ.Item>

  <FAQ.Item question="Why are heat pump quotes so different from each other?">
    Common reasons include different equipment brands and efficiency tiers, varying warranty terms (5-year vs. 10-year labor warranty), scope differences (some quotes include ductwork and electrical work, others don't), contractor overhead and profit margins, and whether the contractor is factory-authorized. Always compare quotes on an apples-to-apples basis by specifying the same efficiency tier and included work.
  </FAQ.Item>

  <FAQ.Item question="What is the cheapest heat pump to install?">
    The cheapest option is a DIY mini split like the MrCool DIY 4th Gen at $1,400–$2,200 total (no professional installation needed). The cheapest professionally installed option is a single-zone mini split from a value brand like Senville or Cooper & Hunter at $2,500–$3,500 total. For whole-home ducted systems, value brands like Goodman or York start at $4,500–$6,000 installed.
  </FAQ.Item>
</FAQ>

<SourcesBox>
  - [ACCA — Find a Quality Contractor](https://www.acca.org/contractors)
  - [IRS — Energy Efficient Home Improvement Credit (25C)](https://www.irs.gov/credits-deductions/energy-efficient-home-improvement-credit)
  - [DSIRE — Database of State Incentives](https://www.dsireusa.org/)
  - [AHRI — Certified Equipment Directory](https://www.ahridirectory.org/)
  - [HomeAdvisor / Angi — HVAC Cost Data (2026)](https://www.angi.com/)
</SourcesBox>

<RelatedArticles articles={frontmatter.relatedArticles} />
