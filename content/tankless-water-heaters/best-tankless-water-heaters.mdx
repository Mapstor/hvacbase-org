---
slug: "best-tankless-water-heaters"
title: "Best Tankless Water Heaters in 2026 (Based on Specs, Not Sponsors)"
description: "The 12 best tankless water heaters in 2026, ranked by UEF efficiency, flow rate, and manufacturer-published performance data. Gas condensing, non-condensing, and electric models compared by specs."
cluster: "tankless-water-heaters"
role: "hub"
priority: "P1"
contentType: "TRANSFORM"
datePublished: "2026-02-05"
dateModified: "2026-07-17"
relatedArticles:
  - "tankless-water-heater-guide"
  - "best-tankless-gas-water-heaters"
  - "best-electric-tankless-water-heaters"
  - "tankless-water-heater-cost"
  - "electric-vs-gas-tankless"
  - "is-tankless-water-heater-worth-it"
  - "smallest-tankless-water-heaters"
externalLinks:
  - label: "ENERGY STAR — Most Efficient Water Heaters 2026"
    url: "https://www.energystar.gov/products/water_heaters"
  - label: "AHRI — Certified Product Directory"
    url: "https://www.ahridirectory.org"
---

import { TableOfContents, FAQ, ComparisonTable, Callout, SourcesBox, RelatedArticles } from '@/components'

# Best Tankless Water Heaters in 2026 (Based on Specs, Not Sponsors)

The best overall tankless water heater in 2026 is the **Navien NPE-2 240S** — a condensing gas unit with a 0.96 UEF, 11.2 GPM max flow rate, and built-in recirculation pump, all for around $1,600–$1,900. For electric, the **EcoSmart ECO 36** leads with 36 kW of heating power, a 0.99 UEF, and a price under $700.

Rankings are based on manufacturer specifications and AHRI-certified efficiency ratings. Below are 12 models across three categories.

<TableOfContents />

## Methodology

A weighted scoring system based on five objective criteria, all sourced from AHRI-certified data or manufacturer datasheets:

1. **UEF (30% weight):** Higher efficiency means lower operating costs. We used AHRI-certified values only.
2. **Flow rate at 77°F rise (25%):** This is the realistic cold-climate performance metric, not the inflated 35°F-rise number.
3. **Warranty (20%):** Heat exchanger warranty length, parts warranty, and labor coverage.
4. **Installed cost-per-GPM (15%):** Total estimated installed cost divided by GPM at 77°F rise — the true value metric.
5. **Features (10%):** Built-in recirculation, Wi-Fi monitoring, error code diagnostics, and modulation range.

All prices reflect 2026 market averages from major retailers and licensed installers.

## Best Gas Condensing Tankless Water Heaters

These are the top-tier, whole-home workhorses. Condensing technology recovers heat from exhaust gases, pushing UEF above 0.90 and allowing cheap PVC venting.

<ComparisonTable
  title="Top Gas Condensing Tankless Water Heaters (2026)"
  headers={["Model", "BTU/h", "UEF", "GPM @ 35°F Rise", "GPM @ 77°F Rise", "Warranty (HX/Parts)", "Street Price"]}
  rows={[
    ["Navien NPE-2 240S", "199,900", "0.96", "11.2", "5.4", "15yr / 5yr", "$1,600–$1,900"],
    ["Rinnai RU199iN", "199,000", "0.93", "11.0", "5.0", "12yr / 5yr", "$1,800–$2,100"],
    ["Noritz EZ111", "199,900", "0.97", "11.1", "5.3", "12yr / 5yr", "$1,700–$2,000"],
    ["Navien NPE-2 180S", "180,000", "0.96", "10.1", "4.8", "15yr / 5yr", "$1,400–$1,700"],
    ["Rinnai RU160iN", "160,000", "0.93", "9.0", "4.2", "12yr / 5yr", "$1,400–$1,650"],
  ]}
/>

### 1. Navien NPE-2 240S — Best Overall Gas Tankless

The NPE-2 series is Navien's second generation, and the 240S variant is the flagship. At 199,900 BTU/h with a 0.96 UEF, it matches or beats every competitor on raw efficiency.

What sets it apart is the built-in recirculation pump with NaviLink Wi-Fi control. You get instant hot water at every tap without buying a separate pump ($200–$400 savings), and you can monitor runtime, error codes, and energy usage from your phone. The ComfortFlow system learns your usage patterns and pre-circulates hot water before peak times.

The 15-year heat exchanger warranty is the longest in the category. Navien's dual stainless steel heat exchangers resist corrosion better than single-exchanger designs, which matters in areas with aggressive water chemistry.

**Best for:** Whole-home applications in any climate, families of 3–6, homeowners who want smart-home integration.

### 2. Rinnai RU199iN — Best Track Record

Rinnai essentially invented the residential tankless market in the U.S., and the RU199iN reflects decades of refinement. The 0.93 UEF is slightly below Navien and Noritz, but Rinnai's reliability reputation is unmatched — their field failure rate is among the lowest in the industry.

The ThermaCirc360 recirculation system is available as an add-on ($200–$300). The unit supports both indoor and outdoor installation (outdoor model: RU199eN) without a separate outdoor enclosure. The scale detection system alerts you when descaling is needed, preventing the #1 cause of premature failure.

**Best for:** Homeowners who prioritize long-term reliability and extensive dealer networks for service.

### 3. Noritz EZ111 — Best Efficiency

The EZ111 achieves a category-leading 0.97 UEF — the highest listed UEF among residential gas tankless units in ENERGY STAR and manufacturer specification data. It uses a dual fire-tube heat exchanger design that squeezes every possible BTU from combustion gases.

The EZLink Wi-Fi module provides remote monitoring and diagnostics. The EZ111 also features a wider modulation range (18,000–199,900 BTU/h, or 11:1 turndown ratio), meaning it adjusts more precisely to low-demand situations. This prevents the "cold water sandwich" effect that cheaper units struggle with.

**Best for:** Homeowners focused on maximum energy savings and tax credit qualification (UEF ≥ 0.95 requirement).

<Callout type="tip">
**Tax credit note (EXPIRED):** Through Dec 31, 2025, all three condensing models above qualified for the 30% federal Section 25C tax credit (up to $2,000 per year) because they exceed the 0.95 UEF threshold. On a $1,800 unit, that was a $540 credit — effectively paying for the efficiency premium over non-condensing models. **The credit expired for property placed in service after Dec 31, 2025 under the OBBBA** (PL 119-21, signed July 4, 2025). For 2026 installs, state/utility rebates plus IRA HEAR (income-qualified, up to $1,750 for water heaters) and HOMES (open to all incomes, performance-based) remain active.
</Callout>

## Best Gas Non-Condensing Tankless Water Heaters

Non-condensing units cost $400–$800 less than condensing, but they require more expensive stainless steel venting and operate at a lower 0.80–0.85 UEF. They make sense when you need to replace an existing non-condensing unit (venting already in place) or have a very tight budget.

<ComparisonTable
  title="Top Gas Non-Condensing Tankless Water Heaters (2026)"
  headers={["Model", "BTU/h", "UEF", "GPM @ 35°F Rise", "GPM @ 77°F Rise", "Warranty (HX/Parts)", "Street Price"]}
  rows={[
    ["Rinnai V94iN", "199,000", "0.82", "9.8", "4.5", "12yr / 5yr", "$1,000–$1,250"],
    ["Rheem RTG-84XLN-3", "180,000", "0.82", "8.4", "4.0", "12yr / 5yr", "$900–$1,100"],
    ["Noritz NRC83", "157,000", "0.82", "8.3", "3.8", "12yr / 5yr", "$800–$1,000"],
  ]}
/>

### 4. Rinnai V94iN — Best Non-Condensing

The V94iN delivers 9.8 GPM at 35°F rise and 4.5 GPM at 77°F rise — the highest throughput in the non-condensing category. At $1,000–$1,250 before installation, it's also the most cost-effective path to whole-home tankless for budget-conscious homeowners in moderate climates.

The unit is available in both indoor (V94iN) and outdoor (V94eN) configurations. It supports optional recirculation and has a 12-year heat exchanger warranty. The main trade-off vs condensing: you'll spend $200–$500 more on stainless steel venting and lose out on the IRA tax credit.

**Best for:** Budget installs, warm-to-moderate climates, replacing an existing non-condensing unit with stainless venting already in place.

### 5. Rheem RTG-84XLN-3 — Best Value Non-Condensing

Rheem's entry hits a sweet spot at $900–$1,100. The 180,000 BTU/h output and 0.82 UEF are middle-of-the-pack, but Rheem's availability through Home Depot and Lowes makes it easy to source, and their contractor network is extensive.

The RTG-84XLN-3 features a hot-start programming function that reduces the cold-water sandwich effect. The 12-year heat exchanger warranty matches competitors. It lacks Wi-Fi connectivity or built-in recirculation, but at this price point, those are reasonable omissions.

**Best for:** Homeowners who want straightforward replacement with wide parts availability.

## Best Electric Tankless Water Heaters

Electric models dominate three scenarios: point-of-use applications, homes without gas service, and warm climates where low temperature rise makes electric feasible for whole-home use.

<ComparisonTable
  title="Top Electric Tankless Water Heaters (2026)"
  headers={["Model", "Power (kW)", "UEF", "GPM @ 35°F Rise", "GPM @ 77°F Rise", "Breaker Requirement", "Street Price"]}
  rows={[
    ["EcoSmart ECO 36", "36 kW", "0.99", "6.1", "2.7", "4x 40A / 240V", "$550–$700"],
    ["Stiebel Eltron Tempra 36 Plus", "36 kW", "0.99", "6.0", "2.7", "4x 40A / 240V", "$700–$900"],
    ["Rheem RTEX-24", "24 kW", "0.99", "4.1", "1.8", "3x 40A / 240V", "$400–$550"],
    ["EcoSmart ECO 11", "13 kW", "0.98", "2.2", "1.0", "1x 60A / 240V", "$200–$300"],
  ]}
/>

### 6. EcoSmart ECO 36 — Best Electric Whole-Home

At 36 kW, the ECO 36 is the most powerful residential electric tankless available. It delivers 6.1 GPM at 35°F rise, which means it can handle two showers simultaneously in climates where groundwater is above 60°F. In cold climates, it's a solid single-shower, multi-faucet solution.

Self-modulating technology adjusts power draw based on demand, so you're not pulling 150A when only filling a sink. Lifetime warranty on the heat exchanger (for the original owner). The compact 17" × 17" × 3.75" footprint mounts anywhere.

**Best for:** Warm-climate whole-home use (Southwest, Southeast, coastal California), homes without gas lines.

### 7. Stiebel Eltron Tempra 36 Plus — Best Electric Build Quality

Stiebel Eltron is a German manufacturer that's been making tankless heaters since the 1920s. The Tempra 36 Plus matches EcoSmart on specs but adds Advanced Flow Control — when demand exceeds capacity, it reduces flow rate rather than delivering cold water. You get warm water at lower pressure rather than cold water at full pressure.

The build quality is a step above competitors. Copper heating chambers, solid brass fittings, and a 7-year parts warranty. The digital temperature display is precise to 1°F. It costs $150–$200 more than the ECO 36, which buys you reliability and flow management.

**Best for:** Homeowners who want premium build quality and intelligent flow management.

### 8. EcoSmart ECO 11 — Best Point-of-Use

For single-fixture applications — a guest bathroom, an office kitchen, a remote laundry room — the ECO 11 delivers 2.2 GPM on a single 60A circuit. At $200–$300 installed, it's the most cost-effective way to add hot water to a space without running long pipe runs.

The 13 kW draw is manageable for most 200A panels without an upgrade. Installation takes a licensed electrician 1–2 hours. The unit is 11.5" × 8" × 3.75" — smaller than a laptop.

**Best for:** Point-of-use installations, under-sink applications, guest houses, tiny homes.

<Callout type="warning">
**Electric reality check:** Even the most powerful electric tankless (36 kW) only delivers 2.7 GPM at a 77°F rise. In Minneapolis (groundwater 40°F, requiring 80°F rise), that's barely one shower. For cold-climate whole-home use, gas remains the only practical option unless you install multiple electric units.
</Callout>

## Quick Recommendation Matrix

Not sure which category fits your needs? Use this decision matrix:

<ComparisonTable
  title="Which Tankless Water Heater Should You Buy?"
  headers={["Your Situation", "Best Pick", "Why"]}
  rows={[
    ["Family of 4+, cold climate, gas available", "Navien NPE-2 240S", "Highest GPM at high temp rise, recirculation built-in"],
    ["Family of 2–4, moderate climate, gas available", "Rinnai RU160iN", "Right-sized, reliable, strong dealer network"],
    ["Budget whole-home, gas available", "Rinnai V94iN", "Lowest cost-per-GPM, proven platform"],
    ["Whole-home, no gas, warm climate", "EcoSmart ECO 36", "Max electric power, 0.99 UEF, low cost"],
    ["Whole-home, no gas, premium build", "Stiebel Eltron Tempra 36 Plus", "German engineering, Advanced Flow Control"],
    ["Single bathroom or point-of-use", "EcoSmart ECO 11", "Compact, cheap, single-circuit install"],
    ["Replacing existing non-condensing gas", "Rinnai V94iN", "Uses existing stainless venting, easy swap"],
    ["Maximum efficiency / tax credit", "Noritz EZ111", "0.97 UEF — highest in gas category"],
    ["Large home, 4+ bathrooms", "2x Navien NPE-2 in parallel", "Only way to hit 8+ GPM at high temp rise"],
    ["Outdoor / garage install", "Rinnai RU199eN", "Purpose-built outdoor enclosure, no venting needed"],
  ]}
/>

## Real-World Performance: What Spec Sheets Don't Tell You

### The Cold Water Sandwich Problem

When you turn on a faucet, you get cold water (sitting in the pipes), then a slug of hot water (left in the heat exchanger from the last use), then another burst of cold water (new water entering before the burner fully fires), and finally consistent hot water.

Units with wider modulation ranges handle this better. The Navien NPE-2 and Noritz EZ111, with their 11:1 turndown ratios, fire the burner at lower output during the transition, smoothing the temperature fluctuation. Budget units with 3:1 or 5:1 turndown ratios produce a more noticeable sandwich effect.

### Altitude Derating

Gas units lose capacity at elevation. At 5,000 feet, expect roughly 10% derating. At 7,000 feet, it's 15–20%. A 199,000 BTU/h unit in Denver effectively delivers ~175,000 BTU/h. Size accordingly and check the manufacturer's altitude rating — most units are rated to 2,000–3,000 feet without modification and up to 7,000–10,500 feet with an altitude kit.

### Hard Water Impact

In areas with water hardness above 12 GPG (much of the Midwest and Southwest), scale buildup can reduce flow rates by 20–30% within 12 months without descaling. Models with scale-detection alerts (Rinnai, Navien) are worth the premium in hard-water areas. Budget $60–$200/year for descaling services, or $30–$60 for a DIY kit.

### Noise Levels

Gas tankless units generate 50–65 dB when firing at full capacity — roughly the volume of a normal conversation. Condensing units tend to be slightly quieter (50–58 dB) than non-condensing (55–65 dB). Electric units are nearly silent (under 40 dB). If the unit is mounted on a bedroom wall, consider a condensing model or electric for the lowest noise.

## Installation Cost Expectations by Model

<ComparisonTable
  title="Estimated Total Installed Cost (2026)"
  headers={["Model", "Unit Cost", "Typical Install", "Total Installed", "After IRA Credit"]}
  rows={[
    ["Navien NPE-2 240S", "$1,750", "$1,800–$2,500", "$3,550–$4,250", "$3,025–$3,725"],
    ["Rinnai RU199iN", "$1,950", "$1,800–$2,500", "$3,750–$4,450", "$3,165–$3,865"],
    ["Noritz EZ111", "$1,850", "$1,800–$2,500", "$3,650–$4,350", "$3,095–$3,795"],
    ["Rinnai V94iN", "$1,125", "$1,500–$2,200", "$2,625–$3,325", "No credit (UEF < 0.95)"],
    ["EcoSmart ECO 36", "$625", "$1,000–$1,800", "$1,625–$2,425", "$1,437–$2,237"],
    ["Stiebel Eltron Tempra 36 Plus", "$800", "$1,000–$1,800", "$1,800–$2,600", "$1,560–$2,360"],
    ["EcoSmart ECO 11", "$250", "$400–$800", "$650–$1,050", "$575–$975"],
  ]}
/>

<Callout type="info">
**Install costs vary by region.** Labor rates in San Francisco, New York, and Boston run 40–60% higher than national averages. In the South and Midwest, expect to land at the lower end of the ranges above. Always get 3 quotes from licensed installers.
</Callout>

## Key Takeaways

<Callout type="takeaway">
- **Best overall:** Navien NPE-2 240S — 0.96 UEF, 11.2 GPM, built-in recirculation, 15-year warranty
- **Best electric:** EcoSmart ECO 36 — 36 kW, 0.99 UEF, under $700, ideal for warm climates
- **Best value:** Rinnai V94iN — reliable non-condensing at $1,000–$1,250
- **Best efficiency:** Noritz EZ111 — 0.97 UEF, highest gas efficiency available
- **Size by GPM at YOUR temperature rise**, not the inflated max-flow number
- **Federal 25C credit EXPIRED Dec 31, 2025** under the OBBBA — through that date, gas condensing units (UEF ≥ 0.95) qualified for 30% up to $2,000. For 2026 installs, IRA HEAR (income-qualified, up to $1,750 for water heaters) + state/utility rebates remain.
- **Don't skip annual descaling** — it protects your $1,500–$4,000 investment
</Callout>

<FAQ questions={[
  {
    question: "What is the most reliable tankless water heater brand?",
    answer: "Rinnai and Navien are the two most reliable tankless water heater brands. Rinnai has the longest track record in the U.S. market (since 1991) and the widest authorized service network. Navien's NPE-2 series has shown strong early reliability and offers the longest heat exchanger warranty at 15 years. Noritz and Stiebel Eltron round out the top tier."
  },
  {
    question: "Is Navien or Rinnai better?",
    answer: "Both are excellent. Navien wins on efficiency (0.96 vs 0.93 UEF), warranty (15 vs 12 years on heat exchanger), and integrated features (built-in recirculation). Rinnai wins on proven reliability, the largest installer network in North America, and slightly better altitude ratings. For most homeowners, Navien is the better value; for those in rural areas or at high altitude, Rinnai's support network gives an edge."
  },
  {
    question: "How much does it cost to install a tankless water heater?",
    answer: "In 2026, expect $1,500–$4,500 total installed for most residential projects. A gas condensing unit averages $3,200–$4,300 installed; a gas non-condensing unit runs $2,500–$3,300; and an electric whole-home unit costs $1,600–$2,500. Point-of-use electric units can be installed for under $1,000. These figures include unit, labor, materials, and permits."
  },
  {
    question: "Do tankless water heaters qualify for federal tax credits in 2026?",
    answer: "No — the federal Section 25C credit (which covered 30% of cost up to $2,000 per year for high-efficiency water heaters with UEF ≥ 0.95) expired for property placed in service after Dec 31, 2025 under the OBBBA (PL 119-21, signed July 4, 2025). Pre-expiration, most gas condensing models and all electric tankless units qualified; non-condensing gas units (UEF 0.80–0.85) never qualified. For 2026 installs, the active federal pathways are IRA HEAR (income-qualified, up to $1,750 for water heaters) and HOMES (open to all incomes, performance-based). State and utility rebates often stack on top. (Sources: IRS OBBB FAQ; Congress.gov CRS IN12611.)"
  },
  {
    question: "What is the best tankless water heater for a family of 5?",
    answer: "A family of 5 should target 6–8 GPM at their local temperature rise. In warm climates (groundwater 60°F+), a Navien NPE-2 180S or Rinnai RU160iN handles this easily. In cold climates (groundwater below 50°F), you'll need a Navien NPE-2 240S or two smaller units in parallel to maintain hot water across 3+ simultaneous uses."
  },
  {
    question: "What is the best tankless water heater for hard water?",
    answer: "For hard water (above 10 GPG), choose a unit with scale-detection alerts — the Rinnai RU199iN and Navien NPE-2 series both have this feature. Pair the unit with a whole-house water softener and commit to descaling every 6–12 months. Without these precautions, hard water can cut a tankless heater's lifespan from 20 years to 7–10 years."
  },
  {
    question: "Can I replace my tank water heater with a tankless?",
    answer: "Yes, but budget for installation complexity. A gas-to-gas swap requires upsizing the gas line (from 1/2\" to 3/4\"), adding proper venting, and possibly relocating the unit. An electric-to-electric swap may require panel upgrades and new dedicated circuits. Plan for $2,000–$4,500 total for a retrofit, compared to $1,500–$3,500 for new construction where infrastructure is planned in advance."
  },
  {
    question: "Are electric tankless water heaters worth it?",
    answer: "In the right application, absolutely. Electric tankless units are worth it when: (1) you need point-of-use hot water at a single fixture, (2) you live in a warm climate with groundwater above 60°F, (3) your home lacks gas service, or (4) you want zero combustion emissions. They're not worth it for whole-home use in cold climates — the electrical requirements (100–150A) are impractical for most homes."
  },
  {
    question: "How long do tankless water heaters last?",
    answer: "Tankless water heaters last 15–20 years on average, with some units exceeding 25 years in areas with soft water and diligent maintenance. Key longevity factors: annual descaling (especially above 7 GPG hardness), proper sizing to avoid overworking the unit, using a sediment pre-filter, and professional installation with correct gas/electrical specs. Tank heaters, by comparison, last 8–12 years."
  },
  {
    question: "What's the difference between indoor and outdoor tankless water heaters?",
    answer: "Outdoor units are weather-sealed and don't require venting — exhaust gases dissipate directly into the air. This simplifies installation and frees interior space. Indoor units need proper venting (PVC for condensing, stainless for non-condensing) routed through a wall or roof. Outdoor units cost slightly more ($100–$200) and include freeze-prevention features, but they reduce total installed cost by $300–$600 because no venting materials or penetrations are needed."
  }
]} />

<SourcesBox sources={[
  { label: "AHRI — Certified Product Performance Directory", url: "https://www.ahridirectory.org" },
  { label: "ENERGY STAR — Most Efficient 2026 Water Heaters", url: "https://www.energystar.gov/products/water_heaters" },
  { label: "Rinnai — Product Catalog", url: "https://www.rinnai.us/residential/tankless-water-heaters" },
  { label: "Stiebel Eltron — Tempra Plus Series", url: "https://www.stiebel-eltron-usa.com/products/tempra-plus" },
  { label: "IRS — Energy Efficient Home Improvement Credit (25C)", url: "https://www.irs.gov/credits-deductions/energy-efficient-home-improvement-credit" },
]} />

<RelatedArticles />
