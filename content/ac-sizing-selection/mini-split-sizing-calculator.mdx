...
You've narrowed the decision to a mini split — probably for one stubborn room, maybe for the whole house — and every installer quote comes back a different tonnage. You already know the two failure modes: an oversized head that short-cycles and never pulls humidity out of the air, or an undersized head that runs flat out and still loses to the July afternoon sun.

The catch is that mini splits don't come in the half-ton increments a Manual J spreadsheet spits out. They ship in fixed rungs — 6K, 9K, 12K, 15K, 18K, 24K, 30K, 36K — so the real question is which rung your zone actually lands on.

This calculator applies the ACCA Manual J style per-zone method to those fixed BTU sizes. We layered in the published climate-zone multipliers, kitchen and ceiling-height adders, and the AHRI-certified low-temperature heating capacities that separate standard heads from cold-climate models, so the number you get out matches the spec sheet a Mitsubishi, Fujitsu, or Daikin dealer would quote you.

A few things you'll pick up along the way: why the "rated at 47°F" heating figure on most spec sheets overstates real winter output by roughly 41% on a standard head versus 19% on a cold-climate one, why the 60–80% design-day loading rule is the sweet spot for inverter compressors, and why three separate single-zone systems often beat one multi-zone outdoor unit on efficiency. No paid placements — just what the published specs say.
